...
```{r}
data <- read_csv("data/drinks_content.csv")
summary(data)
```
...
```{r}
dataedit <- select(data,Beverage_category,Beverage,Beverage_prep,Calories) %>% 
...
```{r}
dataedit <- mutate(dataedit,calorie_diff = as.numeric(Calories)-135,.after = Calories)
```
...
# We can see that the most calorific categories of drinks that Starbucks offer
#are smoothies, frappuccino blended coffee and signature espresso drinks
#while the least calorific are coffee, shaken iced beverages and classic espresso drinks
```
...
```{r}
overall_avg_calories <- mean(pull(dataedit,Calories))
dataedit %>% 
  group_by(Beverage_prep) %>% 
  summarize(bev_mean=mean(Calories,na.rm=TRUE)) %>% 
  filter(bev_mean>overall_avg_calories)
```
...
#the beverage prep with the least amount of calories was solo, the lowest calorie beverage category was Coffee and as shown above the lowest calorie beverage was Tazo Tea (however as we need a coffee drink then it is the next lowest, brewed coffee).. So judging by the data a Solo Brewed Coffee is what to go for if you are aiming at getting the lowest possible calorie drink. I will not be ordering this at my local Starbucks.
```
