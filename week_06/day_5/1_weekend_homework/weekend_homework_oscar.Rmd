...
```{r}
# H0: The difference between a Jabberwockies length of sleep and the average of all animals is 0
# H1: The difference is more than 0

null_distribution <- msleep %>%
  specify(response = sleep_total) %>%
  hypothesize(null = "point", mu = mean(msleep$sleep_total)) %>%
...
```
The p-value is 0 and therefore we can reject the null hypothesis and accept this as evidence that the jabberwockie has a very different total sleep time compared to the other animal types.
...
```{r}
# H0: The difference in means between a sleep for omnivores and carnivores is 0
# H1: The difference is more than 0

observed_stat <- mean((msleep %>% filter(vore == "carni"))$sleep_total, na.rm = TRUE) - mean((msleep %>% filter(vore == "omni"))$sleep_total, na.rm = TRUE)
...
null_distribution <- msleep %>%
  ...
  calculate(stat = "diff in means", order = c("carni", "omni"))

null_distribution %>% 
  get_p_value(obs_stat = observed_stat, direction = "both")
```
the p-value is fairly significant, meaning that there is not enough evidence to suggest that the alternative to the null hypothesis
...
```{r}
msleep <- msleep %>% 
  mutate(is_dom = case_when(conservation == "domesticated" ~ TRUE,
                            TRUE ~ FALSE))

null_distribution <- msleep %>%
  mutate(is_dom = as.character(is_dom)) %>% 
  specify(response = is_dom, success = "TRUE") %>%
...
observed_stat <- is_dom_count[[2]]/sum(is_dom_count)
...
Answer:
- Proportion Test
- H0: Less than 40% of people in the town have heard of the shop
- H1: More than 40% of people in the town have heard of the shop
- Simulation
